...
const setupRichMenuA = async () => {
  ...
  const richMenu = {
    ...
    areas: [
      ...
      { bounds: { x: 1250, y: 0, width: 1250, height: 201 }, action: { type: "richmenuswitch", richMenuAliasId: "menu_b_v2", data: "switch_to_chat" } },
...
// B面のリッチメニュー作成（商品検索ボタンにpostback指定）
const setupRichMenuB = async () => {
  ...
  try {
    ...
    await lineService.createRichMenuAlias("menu_b_v2", richMenuId);
    console.log("STEP 4: B面エイリアス menu_b_v2 設定完了");
